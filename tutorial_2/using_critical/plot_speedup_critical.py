...
data = np.loadtxt("performance_results_critical.txt", delimiter=",", skiprows=1)
...
if parallel_factors:
    ...
    if(avg_parallel_fraction > 0):
        print(f"Estimated Parallelization Fraction: {avg_parallel_fraction:.4f}")
    max_parallel_fraction = max(parallel_factors)
    if(max_parallel_fraction < 0):
        print(f"Parallelization Fraction is less than 1, which means the code is not parallelized.") 
        print(f"The code run best with 1 thread")
    else:   
